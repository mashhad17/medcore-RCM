...
-- ------------------------------------------------------------
--  AUTHORITATIVE schema the live app runs on (config.php -> medcore_db).
--  lib.php decomposes the front-end's nested appointment objects into
--  these tables on save and reassembles them on read.
--
--  This file is designed directly from the queries in:
--    lib.php, api.php, bootstrap.php,
--    api/walkin.php, api/dashboard/summary.php,
--    api/queue/top-waiting.php, api/activity/clear.php
--  A fresh import gives the EXACT shape every query expects — no
--  reliance on the runtime self-heal in mc_ensure_flow_columns() and
--  no need to run the sql/migrations/*.sql files (their columns are
--  already folded in here and noted with "[folded: ...]").
--
--  Design notes on dates:
...
--      ISO strings from the browser) are kept as VARCHAR so they
--      round-trip byte-for-byte and the existing front-end keeps working.
--
...
-- mc_lookup_doctor(): SELECT id FROM departments WHERE name = ?  /  INSERT (name)
...
--   mc_lookup_doctor():  SELECT id WHERE full_name = ?
--                        SELECT COALESCE(MAX(col_index)+1,0)
--                        INSERT (full_name, department_id, col_index)
--   mc_provider_status(): SELECT d.id, d.full_name, dep.name, d.manual_status
--                         WHERE d.is_active = 1 ORDER BY d.col_index, d.id
CREATE TABLE doctors (
    ...
    manual_status VARCHAR(20)  NULL,        -- [folded: 2026_dashboard_slice] 'On Break'|'Off-Shift'|NULL (NULL = derive)
    created_at    TIMESTAMP    DEFAULT CURRENT_TIMESTAMP,
...
--   mc_save_appointments() findTreat: SELECT id WHERE code = ?
...
--   mc_save_appointments() findPlan: SELECT id WHERE name = ?
...
-- Reception / admin staff (collected_by labels, auth seed).
CREATE TABLE staff (
    id         INT AUTO_INCREMENT PRIMARY KEY,
...
--   mc_upsert_patient():   SELECT id WHERE mrn = ?  /  INSERT  /  UPDATE
--   api/walkin.php:        SELECT 1 WHERE mrn = ?  (collision check)
...
-- The scheduler's core record.
--   mc_get_appointments():  SELECT a.* JOIN patients JOIN doctors ORDER BY created_at, id
--   mc_save_appointments(): INSERT (... sent_to_doctor, portal_appointment_id, booked_duration_min)
--   mc_sync_durations_from_portal(): UPDATE booked_duration_min / duration_min WHERE id = ?
--   dashboard:              WHERE appt_date = ? ; ORDER BY start_hour, start_minute, id
--   api/walkin.php:         INSERT (... appt_date=CURDATE(), status='arrived')
CREATE TABLE appointments (
    ...
    -- [folded: mc_ensure_flow_columns] patient-flow / portal-bridge fields
    sent_to_doctor        TINYINT(1) NOT NULL DEFAULT 0,  -- "Send to Doctor" lifecycle flag
    portal_appointment_id INT        NULL,               -- linked medical_center.appointments id
    booked_duration_min   INT        NULL,               -- original duration before portal extends it
    created_at     TIMESTAMP   DEFAULT CURRENT_TIMESTAMP,
...
-- Vitals captured at a visit (one current row read per appointment).
--   mc_get_appointments(): SELECT ... WHERE appointment_id = ? ORDER BY id LIMIT 1
CREATE TABLE vitals (
    id              INT AUTO_INCREMENT PRIMARY KEY,
...
--   mc_get_appointments(): SELECT * WHERE appointment_id = ? LIMIT 1
--   mc_save_payments():    SELECT id WHERE invoice_no = ?  (best-effort link)
...
--   mc_get_payments():  SELECT pay.* LEFT JOIN patients ORDER BY pay.id
--   mc_save_payments(): DELETE all ; INSERT (...)
...
-- ============================================================
--   mc_get_appointments(): SELECT * WHERE appointment_id = ? ORDER BY consent_form_id
...
-- Live waiting-room queue (transient, "today" board).
--   mc_get_queue():   SELECT lq.* LEFT JOIN patients ORDER BY lq.id
--   mc_purge_stale_queue(): DELETE WHERE LEFT(checked_in_at,10) < CURDATE()
--   mc_next_up()/top-waiting: JOIN patients ORDER BY checked_in_at ASC LIMIT n
--   mc_provider_status(): SELECT DISTINCT doctor_label WHERE stage = 'consultation'
CREATE TABLE live_queue (
    ...
    -- [folded: mc_ensure_flow_columns] live-queue lane + appointment linkage
    stage              VARCHAR(20) NOT NULL DEFAULT 'waiting', -- waiting|consultation|billing
    consult_started_at VARCHAR(40) NULL,
    appt_id            VARCHAR(64) NULL,                       -- source appointment id
    CONSTRAINT fk_queue_patient FOREIGN KEY (patient_id)
        REFERENCES patients(id) ON DELETE CASCADE,
    INDEX idx_queue_patient (patient_id),
    INDEX idx_queue_stage (stage)
) ENGINE=InnoDB;

-- Audit / activity feed (append-only chronological event log).
--   mc_log_event():       INSERT (log_time, message, author, event_type)
--   mc_recent_activity(): SELECT ... ORDER BY created_at DESC, id DESC LIMIT n
--   mc_get_activity():    SELECT * ORDER BY id ASC
CREATE TABLE activity_log (
    id         INT AUTO_INCREMENT PRIMARY KEY,
    log_time   VARCHAR(40),
    message    VARCHAR(255) NOT NULL,
    author     VARCHAR(80),
    event_type VARCHAR(40)  NULL,                          -- [folded: 2026_dashboard_slice]
    created_at TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP, -- [folded: 2026_dashboard_slice] machine sort key
