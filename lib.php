...
/**
 * Pull consultation COMPLETION back from the Doctor Portal (medical_center DB).
 *
 * When a doctor finalises a consult on the portal it sets that appointment's
 * status to 'Completed' (doctor_portal/actions/save_consultation.php). Reception
 * links each sent consult to its portal row via `ext_ref` (= the medcore
 * appointment id), so we match on that and advance the patient's live-queue lane
 * from Consultation → Billing & Checkout. Reception then collects payment, which
 * removes them from the queue (collectPayment / "Mark as Paid").
 *
 * This is the completion counterpart to mc_sync_durations_from_portal(): the two
 * apps stay independent and reception PULLS state from the portal. Idempotent —
 * once a row reaches 'billing' it no longer matches, so it advances (and logs)
 * exactly once. Silent no-op if the portal DB isn't reachable.
 */
function mc_sync_status_from_portal($pdo)
{
    mc_ensure_flow_columns($pdo);
    try {
        $portal = new PDO(
        ...
        $done = $portal->query(
            "SELECT ext_ref FROM appointments
             WHERE ext_ref IS NOT NULL AND ext_ref <> '' AND status = 'Completed'"
        )->fetchAll();
    } catch (Throwable $e) {
        return; // portal not set up / unreachable — leave reception data as-is
    }

    // Move each completed consult's queue entry into the Billing lane (once).
    $who = $pdo->prepare(
        "SELECT p.full_name AS name, lq.doctor_label AS doctor
         FROM live_queue lq JOIN patients p ON lq.patient_id = p.id
         WHERE lq.appt_id = ? AND lq.stage = 'consultation' LIMIT 1"
    );
    $advance = $pdo->prepare(
        "UPDATE live_queue SET stage = 'billing'
         WHERE appt_id = ? AND stage = 'consultation'"
    );
    foreach ($done as $r) {
        $ext = (string)$r['ext_ref'];
        if ($ext === '') continue;
        // Capture who BEFORE the update so we can log a meaningful message.
        $who->execute([$ext]);
        $info = $who->fetch();
        $advance->execute([$ext]);
        if ($advance->rowCount() > 0 && $info) {
            $name = $info['name'] ?: 'Patient';
            $doc  = $info['doctor'] ? preg_split('/\s*\(/', $info['doctor'])[0] : 'the doctor';
            mc_log_event($pdo, "$name — consultation completed by $doc → Billing", 'Doctor Portal', 'consult-done');
        }
    }
}
...
// Live patient-flow list for the dashboard, with server-computed minutes. Each
// row carries its live queue `stage` so the dashboard mirrors the Live Clinic
// Queue exactly across the whole journey: Waiting → In Consultation → Billing.
// A patient never silently disappears mid-flow — they stay visible (as "Ready
// for Checkout") until reception collects payment and they leave the queue.
// Ordered by stage (waiting → consultation → billing), then by check-in time.
// Wait minutes count from check-in while waiting, and from consult start once
// in consultation.
function mc_next_up($pdo, $limit = 5)
{
    ...
    $rows = $pdo->query(
        "SELECT p.full_name AS patient_name, q.doctor_label AS doctor_name,
                q.checked_in_at, q.stage, q.consult_started_at
         FROM live_queue q
         JOIN patients p ON q.patient_id = p.id
         WHERE LEFT(q.checked_in_at, 10) = CURDATE()
         ORDER BY FIELD(COALESCE(q.stage, 'waiting'), 'waiting', 'consultation', 'billing'),
                  q.checked_in_at ASC
         LIMIT $limit"
    )->fetchAll();
    ...
    foreach ($rows as $r) {
        $stage = $r['stage'] ?: 'waiting';
        // While waiting, time accrues from check-in; once in consultation it
        // measures how long the consult has been running.
        $ref = ($stage === 'consultation' && !empty($r['consult_started_at']))
